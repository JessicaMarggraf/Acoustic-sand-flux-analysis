...
def user_input_path_samples():
    root = tk.Tk()
    root.withdraw()
    root.attributes("-topmost", True)
    filepath_stage = str(tk.filedialog.askdirectory(title='Define path folder with sampling data'))
    filepath_stage = filepath_stage.replace('(', '').replace(")", '').replace("'", '').replace(",", '').replace("/", "\\")
    root.destroy()    
    return filepath_stage
...
def user_input_path_freq():
    root = tk.Tk()
    root.withdraw()
    root.attributes("-topmost", True)
    filepath_stage = str(tk.filedialog.askdirectory(title='Define path of all acoustic intensity and background data'))
    filepath_stage = filepath_stage.replace('(', '').replace(")", '').replace("'", '').replace(",", '').replace("/", "\\")
    root.destroy()    
    return filepath_stage
